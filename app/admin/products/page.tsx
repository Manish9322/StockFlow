...
type SortField = "name" | "quantity" | "costPrice" | "sellingPrice" | "updatedAt" | "user"
...
function AdminProductsContent() {
  ...
  const users = useMemo(() => {
    return Array.from(new Set(products.map((p) => p.userId).filter(Boolean)))
      .map(user => ({
        _id: user._id,
        name: user.name,
        email: user.email,
  ...
  const filteredProducts = products
    .filter((product) => {
      const matchesSearch =
        ...
        product.userId?.email?.toLowerCase().includes(searchQuery.toLowerCase())
      const matchesCategory = selectedCategory === "all" || product.category?.name === selectedCategory
      const matchesUser = selectedUser === "all" || product.userId?._id === selectedUser
      ...
      return matchesSearch && matchesCategory && matchesUser && matchesStockStatus && matchesPriceRange
    })
    .sort((a, b) => {
          ...
          <div className="flex flex-col lg:flex-row gap-3 md:gap-4 items-stretch lg:items-end">
            <div className="flex-1 min-w-0">
              ...
              <div className="relative">
                ...
                />
                <Input
                  placeholder="Search by product name, SKU, user name or email..."
                  value={searchQuery}
            ...
            <div className="inline-block min-w-full px-4 md:px-0">
              {isLoading ? (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-8 gap-4 pb-3 border-b border-border">
                    ...
                    <div className="h-4 bg-linear-to-r from-muted via-muted/60 to-muted rounded animate-pulse hidden md:block"></div>
                  ...
                  {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
                    <div key={i} className="grid grid-cols-8 gap-4 py-4 border-b border-border/50 items-center">
                      ...
                      <TableRow key={product._id}>
                        ...
                        <TableCell className="text-xs md:text-sm hidden sm:table-cell">
                          <div className="flex flex-col">
                            <span className="font-medium">{product.userId?.name || "N/A"}</span>
