...
function ProductDetailContent({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
      <MainLayout>
        <div className="p-4 md:p-8">
          <Link href="/admin/products" className="inline-flex items-center gap-2 text-primary hover:underline text-sm md:text-base mb-4">
            <ChevronLeft size={18} />
            Back to Products
          </Link>
  ...
  return (
    <MainLayout>
      <div className="p-4 md:p-8 space-y-6 md:space-y-8">
        <Link href="/admin/products" className="inline-flex items-center gap-2 text-primary hover:underline text-sm md:text-base">
          <ChevronLeft size={18} />
          Back to Products
        </Link>
        ...
        <div>
          ...
          {product.userId && (
            <p className="text-xs md:text-sm text-muted-foreground mt-1">
              Owner: <span className="font-medium">{product.userId.name}</span>
              {product.userId.company && <span> ({product.userId.company})</span>}
              {product.userId.email && <span className="text-xs"> • {product.userId.email}</span>}
            </p>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
          <div className="bg-card border border-border rounded-lg p-4 md:p-6 space-y-4">
            ...
            <div className="space-y-3 text-xs md:text-sm">
              {product.userId && (
                <>
                  <div className="flex justify-between gap-2 pb-3 border-b border-border">
                    <span className="text-muted-foreground">Product Owner</span>
                    <div className="flex flex-col items-end">
                      <span className="font-medium text-foreground text-right">{product.userId.name}</span>
                      {product.userId.company && (
                        <span className="text-xs text-muted-foreground">{product.userId.company}</span>
                      )}
                      {product.userId.email && (
                        <span className="text-xs text-muted-foreground">{product.userId.email}</span>
                      )}
                    </div>
                  </div>
                </>
          ...
          <div className="bg-card border border-border rounded-lg p-4 md:p-6 space-y-4">
            <h2 className="font-semibold text-foreground text-sm md:text-base">Admin Actions</h2>
            <div className="space-y-3">
              <Link href="/admin/products" className="block">
                <Button className="w-full">Back to All Products</Button>
              </Link>
